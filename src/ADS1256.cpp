...
void ADS1256::init(int8_t _pinCS, int8_t _pinRDY, int8_t _pinRESET, uint32_t _speedSPI)
{
	if (_pinCS == -1 || _pinRDY == -1 || _pinRESET == -1 || _speedSPI == -1)
	{
	}
	else
	{
	...
	delay(500);
	SPI.begin(pinSCLK, pinDOUT, pinDIN, pinCS); // start the spi-bus
	delay(500);
...
void ADS1256::readInputToAdcValuesArrayChannel(int8_t Channel)
{
	...
	else
	{
		Channel -= 1;
		// read all 8 Single Ended Channels AINx-AINCOM
		byte channel = mux[Channel]; // analog in channels #
		...
		SPI.transfer(0x01);
		// Read Data 0000  0001 (01h)       // ********** Step 3 **********
		delayMicroseconds(5);

		adc_Raws[Channel] = SPI.transfer(0);
		adc_Raws[Channel] <<= 8; // shift to left
		adc_Raws[Channel] |= SPI.transfer(0);
		adc_Raws[Channel] <<= 8;
		adc_Raws[Channel] |= SPI.transfer(0);
		delayMicroseconds(2);
		...
		// Single ended Measurements
		if (adc_Raws[Channel] > 0x7fffff)
		{													  // if MSB == 1
			adc_Raws[Channel] = adc_Raws[Channel] - 16777216; // do 2's complement
		}
		adcValues[Channel] = adc_Raws[Channel];
	}
}

void ADS1256::Reset(int8_t _CS)
{
